...
import orngEnviron, utils.widgetparser
...
def readWidgets(directory, cachedWidgetDescriptions, prototype=False, silent=False, addOn=None, defaultCategory=None):
    ...
    for filename in glob.iglob(os.path.join(directory, "*.py")):
        ...
        try:
            meta = utils.widgetparser.WidgetMetaData(data, defaultCategory, enforceDefaultCategory=prototype)
        ...
        try:
            ...
            outputClasses = set(eval(x[1], wmod.__dict__).__name__ for x in eval(meta.outputList))
            #set(y.__name__ for x in eval(meta.outputList) for y in eval(x[1], wmod.__dict__).mro())
