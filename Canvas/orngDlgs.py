...
#            self.inWidgetIcon.setPos(xWidgetOff + xSpaceBetweenWidgets + 2*width - xIconOff - self.inWidgetIcon.pixmap().width(), yWidgetOffTop + height/2.0 - self.inWidgetIcon.pixmap().width()/2.0)
    ...
    def addLink(self, outName, inName):
        ...
        for (name, box) in self.outBoxes:
            if type(name) == type(outName) and name == outName: outBox = box
        for (name, box) in self.inBoxes:
            if type(name) == type(inName) and name == inName: inBox  = box
    ...
    def removeLink(self, outName, inName):
        for (line, outN, inN, outBox, inBox) in self.lines:
            if type(outN) == type(outName) and outN == outName and type(inN) == type(inName) and inN == inName:
                line.hide()
...
class SignalDialog(QDialog):
    ...
    def addLink(self, outName, inName):
        ...
        if not issubclass(outType, inType) and not issubclass(inType, outType): return 0 

        inSignal = None
        inputs = self.inWidget.widgetInfo.inputs
        for i in range(len(inputs)):
            if type(inputs[i].name) == type(inName) and inputs[i].name == inName: inSignal = inputs[i]

        # if inName is a single signal and connection already exists -> delete it
        for (outN, inN) in self._links:
            if type(inN) == type(inName) and inN == inName and inSignal.single:
...
class AddOnManagerDialog(QDialog):
    ...
    def refreshInfoPane(self, item=None):
        ...
        if hasattr(item, "newest"):
            if item.newest.__class__ is not Orange.utils.addons.OrangeRegisteredAddOn:
                ...
                self.lblVerInstalledValue.setText((addOnInstalled.version_str+("" if addOnInstalled.directory.startswith(orngEnviron.addOnsDirUser) else " (installed system-wide)")) if addOnInstalled else "-")
    ...
    def addAddOnsToTree(self, repoItem, addOnDict, insertToBeginning=False):
        ...
        if self.sortSingleLast:
            addOnList = [(n, v) for (n, v) in addOnList if not n.has_single_widget] \
    ...
    def addRepositoryToTree(self, repo):
        ...
        addOnsToAdd = {}
        visibleAddOns = repo.search_index(self.searchStr)
    ...
    def refreshView(self, selectedRegisteredAddOnId=None):
        ...
        if self.groupByRepo:
            for repo in self.repositories:
                ...
                shownAddOns = shownAddOns.union(set(repo.addons).intersection(repo.search_index(self.searchStr)))
        else:
            addOns = {}
            for repo in self.repositories:
                for addOnId in repo.addons:
                    if addOnId in repo.search_index(self.searchStr):
