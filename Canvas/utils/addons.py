...
class OrangeRegisteredAddOn():
    ...
    def prepare(self, id=None, name=42, version="auto", description=None,
        ...
        orangecss = os.path.join(environ.docDir, "style.css")
        ...
        import shutil
        iconbg_file = os.path.join(environ.picsDir, "background_32.png")
        iconun_file = os.path.join(environ.picsDir, "Unknown.png")
...
def repository_list_filename():
    ...
    """
    orange_settings_dir = os.path.realpath(environ.settingDir)
    ...
    if not os.path.isfile(list_file_name):
        ...
        try:
            canvas_settings_dir = os.path.realpath(environ.settingDir)
...
def load_addons():
    ...
    """
    load_installed_addons_from_dir(utils.environ.add_ons_dir)
    load_installed_addons_from_dir(utils.environ.add_ons_dir_user)
    pass
...
def __read_addon_lists(user_only=False):
    return __read_addons_list(os.path.join(utils.environ.orange_settings_dir, "add-ons.txt"),
                              False) + ([] if user_only else
                                        __read_addons_list(os.path.join(utils.environ.install_dir, "add-ons.txt"),
                                                           True))

def __write_addon_lists(addons, user_only=False):
    file(os.path.join(utils.environ.orange_settings_dir, "add-ons.txt"), "wt").write("\n".join(["%s\t%s" % (a.name, a.directory) for a in addons if not a.systemwide]))
    if not user_only:
        file(os.path.join(utils.environ.install_dir        , "add-ons.txt"), "wt").write("\n".join(["%s\t%s" % (a.name, a.directory) for a in addons if     a.systemwide]))
